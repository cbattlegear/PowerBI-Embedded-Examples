...
namespace AppOwnsData.Services
{
    ...
    public class PbiEmbedService
    {
        ...
        public EmbedParamsReport GetEmbedParams(Guid workspaceId, Guid reportId, [Optional] Guid additionalDatasetId)
        {
            ...
            // We need this for an RDL report that points to a XMLA/PBI dataset
            ...
            if (isRDLReport)
            {
                embedToken = GetEmbedTokenRDL(reportId, datasetIds, workspaceId);
            }
            else
            {
            ...
            var embedParams = new EmbedParamsReport
            {
        ...
        public EmbedParamsReport GetEmbedParamsRls(Guid workspaceId, Guid reportId, string rlsUsername, string rlsRole, [Optional] Guid additionalDatasetId)
        {
            PowerBIClient pbiClient = this.GetPowerBIClient();
            ...
            {
                embedToken = GetEmbedToken(reportId, Guid.Parse(pbiReport.DatasetId), rlsUsername, rlsRole, workspaceId);
            }
            //}
        ...
        public EmbedParamsCreate GetEmbedParamsCreate(Guid workspaceId, Guid datasetId)
        {
            PowerBIClient pbiClient = this.GetPowerBIClient();

            // Get dataset info
            var pbiDataset = pbiClient.Datasets.GetDatasetInGroup(workspaceId, datasetId.ToString());

            EmbedToken embedToken;

            embedToken = GetEmbedCreateToken(datasetId, workspaceId);

            // Add report data for embedding
            var embedCreates = new List<EmbedCreate>() {
                new EmbedCreate
                {
                    DatasetId = Guid.ParseExact(pbiDataset.Id, "D"), EmbedUrl = pbiDataset.CreateReportEmbedURL
                }
            };

            // Capture embed params
            var embedParams = new EmbedParamsCreate
            {
                EmbedCreate = embedCreates,
                Type = "Create",
                EmbedToken = embedToken
            };
        ...
        public EmbedParamsQnA GetEmbedParamsQnA(Guid workspaceId, Guid datasetId)
        {
            PowerBIClient pbiClient = this.GetPowerBIClient();
            ...
            embedToken = GetEmbedQnAToken(datasetId);

            // Add report data for embedding
            var embedQnAs = new List<EmbedQnA>() {
                new EmbedQnA
                {
                    DatasetIds = new List<Guid> { Guid.ParseExact(pbiDataset.Id, "D") }, EmbedUrl = pbiDataset.QnaEmbedURL
                }
            };

            // Capture embed params
            var embedParams = new EmbedParamsQnA
            {
                EmbedQnA = embedQnAs,
                Type = "qna",
                EmbedToken = embedToken
        ...
        public EmbedParamsReport GetEmbedParams(Guid workspaceId, IList<Guid> reportIds, [Optional] IList<Guid> additionalDatasetIds)
        {
        ...
        /// <returns>Embed token</returns>
        /// <remarks>This function is not supported for RDL Report</remakrs>
        public EmbedToken GetEmbedTokenRDL(Guid reportId, IList<Guid> datasetIds, [Optional] Guid targetWorkspaceId)
        {
            PowerBIClient pbiClient = this.GetPowerBIClient();
        ...
        public EmbedToken GetEmbedCreateToken(Guid datasetId, [Optional] Guid targetWorkspaceId)
        {
            PowerBIClient pbiClient = this.GetPowerBIClient();
        ...
        public EmbedToken GetEmbedQnAToken(Guid datasetId)
        {
            PowerBIClient pbiClient = this.GetPowerBIClient();
        ...
        public EmbedToken GetEmbedToken(Guid reportId, Guid datasetId, string userName, string rlsRole, [Optional] Guid targetWorkspaceId)
        {
            PowerBIClient pbiClient = this.GetPowerBIClient();
