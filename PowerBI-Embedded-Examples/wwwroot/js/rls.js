...
function refreshTimeout() {
    if (countdown <= 0) {
        countdown = 20;
        $('#timeout').text('');
        $('#rlsuser').prop("disabled", false);
    } else {
        countdown = countdown - 1;
        $('#timeout').text('Refreshing Rate Limited, wait ' + countdown + ' seconds.');
        $('#rlsuser').prop("disabled", true);
        setTimeout(refreshTimeout, 1000);
    }
}

async function rls(username) {
    var models = window["powerbi-client"].models;
    var reportContainer = $("#report-container").get(0);
    $.ajax({
        type: "GET",
        url: "/embedinfo/getembedreportrlsinfo?rlsusername=" + username + "&rlsrole=Provider",
